        ...
        while len(self.clients) < 2:
            try:
                ...
                try:
                    ...
                    if not data:
                        print(f"[猜數字對戰伺服器] 連線已關閉 (可能是探測連線)")
                        client_socket.close()
                        continue
                    ...
                    if msg.get("type") == "ready":
                        ...
                        print(f"[猜數字對戰伺服器] 玩家 {player_id+1} 已加入")
                    else:
                        print(f"[猜數字對戰伺服器] 收到無效握手訊息")
                        client_socket.close()
                except socket.timeout:
                    print(f"[猜數字對戰伺服器] 等待握手超時")
                    client_socket.close()
                except Exception as e:
                    print(f"[猜數字對戰伺服器] 握手錯誤: {e}")
                    client_socket.close()
                    
            except Exception as e:
                print(f"[猜數字對戰伺服器] Accept 錯誤: {e}")
    ...
    def run_game(self):
        ...
        for client in self.clients:
            try:
                ...
                print(f"[錯誤] 發送設定請求失敗: {e}")

        # 依序接收玩家設定
        for i in range(2):
            try:
                # 如果玩家已經發送了數據（因為我們已經發送了請求），這裡會讀取到
                # 如果還沒發送，這裡會等待
                data = self.clients[i].recv(4096).decode()
                message = json.loads(data)
                if message["type"] == "number_set":
                    self.player_numbers[i] = message["number"]
                    print(f"[猜數字對戰伺服器] 玩家 {i+1} 已設定數字: {self.player_numbers[i]}")
            except Exception as e:
                print(f"[錯誤] 接收玩家 {i+1} 設定失敗: {e}")
                return
